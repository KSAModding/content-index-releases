...
def check_dependencies(base, head, errors, owner_only, derived=None):
    ...
    named = alternatives | {key[1] for key in head_entries if key[0] == "id"}
    for key in sorted(set(base_entries) - set(head_entries), key=str):
        ...
        if before.get("source") != "derived":
            ...
            _check_declared(key, named, derived, errors)
...
def _check_declared(key, named, derived, errors):
    ...
    if derived is None:
        ...
        return
    declared = {entry["id"].lower() for entry in derived}
    names = key[1] if key[0] == "any_of" else (key[1],)
    for name in names:
        if name in declared and name not in named:
            errors.append(
